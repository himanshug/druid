...
import io.druid.indexing.overlord.hrtr.HttpRemoteTaskRunner;
    ...
    synchronized (lock) {
      ...
      for (TaskAnnouncement announcement : workerCuratorCoordinator.getAnnouncements()) {
        if (announcement.getTaskStatus().isRunnable()) {
          ...
          TaskAnnouncement completedAnnouncement = completed.get(announcement.getTaskId());
          if (completedAnnouncement != null) {
            completionStatus = completedAnnouncement.getTaskStatus();
          } else if (!running.containsKey(announcement.getTaskStatus().getId())) {
