...
public class K8sDiscoveryModule implements DruidModule
{
  ...
  public void configure(Binder binder)
  {
    ...
    binder.bind(K8sApiClient.class).to(DefaultK8sApiClient.class);
    binder.bind(K8sLeaderElectorFactory.class).to(DefaultK8sLeaderElectorFactory.class);
    ...
    PolyBind.optionBinder(binder, Key.get(DruidLeaderSelector.class, Coordinator.class))
            .addBinding(K8S_KEY)
            .toProvider(
                new DruidLeaderSelectorProvider("coordinator")
            )
            .in(LazySingleton.class);
    ...
    PolyBind.optionBinder(binder, Key.get(DruidLeaderSelector.class, IndexingService.class))
            .addBinding(K8S_KEY)
            .toProvider(
                new DruidLeaderSelectorProvider("overlord")
            )
            .in(LazySingleton.class);
  }
  ...
  private static class DruidLeaderSelectorProvider implements Provider<DruidLeaderSelector>
  {
    ...
    private String lockResourceNameSuffix;

    DruidLeaderSelectorProvider(String lockResourceNameSuffix)
    {
      this.lockResourceNameSuffix = lockResourceNameSuffix;
    }
    ...
    public DruidLeaderSelector get()
    {
      return new K8sDruidLeaderSelector(
          druidNode,
          discoveryConfig.getClusterIdentifier() + "-leaderelection-" + lockResourceNameSuffix,
          podInfo,
          discoveryConfig,
